...
argp.add_argument("-n", action="store", dest="name", required=True, help='source name for folders')
argp.add_argument("-i", action="store", dest="input_data_path", required=True, help='input_data_path')
argp.add_argument("-o", action="store", dest="output_storage_path", required=True, help='output_storage_path')
argp.add_argument("-l", action="store", dest="load", required=False, default=1,
                  help='load dataset? set to 0 to generate (real) dataset pickle files from scratch (which adds ~20 min per job)',
                  type=int)
argp.add_argument("-s", action="store", dest="N_sets", required=True,
                  help='No. of simulated sets to generate and analyse per job, keep ~10', type=int)
argp.add_argument("-c", action="store", dest="config_path", required=False,
                  help='path to gammapy analysis config file, needed if load=0', type=int)

argp.add_argument("--RA", action="store", dest="src_RA", required=True, help='Source RA', type=float)
argp.add_argument("--DEC", action="store", dest="src_DEC", required=True, help='Source DEC', type=float)
argp.add_argument("--da-cut", action="store", dest="da_cut", required=False, default=0.5, type=float,
                  help='max da_cut applied to speed up search')
argp.add_argument("--dt-max", action="store", dest="dt_threshold", required=False, default=1e9, type=float,
                  help='max dt_cut applied to speed up search')
argp.add_argument("--multi_N", action="store", dest="multi_N", required=False, default=2, type=int,
                  help='N for N-multiplet to search for')
argp.add_argument("--r-area-cut", action="store", dest="r_area", required=False, default=1, type=float,
                  help='max r_area around source to search in')
args = argp.parse_args()

if args.config_path is not None:  # TODO, IMPLEMENT
    raise Exception("Feature not implemented, please do not use '--config-file' parameter in current version")

load, N_sets, da_cut, dt_threshold, multi_N, r_area, src_DEC, src_RA = args.load, args.N_sets, args.da_cut, args.dt_threshold, args.multi_N, args.r_area, args.src_DEC, args.src_RA

data_store = DataStore.from_dir(args.input_data_path)
dump_path = Path(args.output_storage_path) / 'real_dataset_dumps' / f'{args.name}'
dump_path_lists = Path(
    args.output_storage_path) / 'MC_dumps' / f'{args.name}' / f'N{multi_N}_da({da_cut})_dt({dt_threshold / 1e6} ms)'
...
for N_set in tqdm(range(0, N_sets)):
    ...
    table = scanner_sim(position_src, mc_events, dt_threshold, r_deg=da_cut, r_area=r_area, Nmin=multi_N, Nmax=multi_N)
