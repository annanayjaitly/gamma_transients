...
def multiplet_scanner(target, events_lists, dt_threshold, r_deg, r_area, Nmin: int = 2, Nmax: int = None,
                      verbose=False):
    ...
    if target is not None:
        ...
        for counter, event_list in enumerate(events_lists):
            ...
            events_table['fTIME'] = events_table['TIME'].copy().astype(np.float64)  # create a float64 time data column

            events_table = events_table[
                target.separation(events_table["coord"]).degree <= r_area]  # select events within r_area of source
            ...
            photons = [Photon(t=event['fTIME'], ra=event['RA'], dec=event['DEC'], id_=event['EVENT_ID']) for event in
                       events_table] 
            ...
            sep_matrix = np.triu(coord_vector.separation(coord_vector.reshape(-1, 1)).degree, k=1)

            doublet_ids = np.argwhere((dt_matrix <= dt_threshold) & (dt_matrix > 0) & (sep_matrix <= da_cut) & (
                    sep_matrix > 0))  # find doublets, correspond to first and last event in multiplet if Nmax >2
            ...
            multiplets_max = f_N(photons,
                                [[photons[id1], photons[id2]] for id1, id2 in zip(doublet_ids1, doublet_ids2)], 
                                dt_threshold, r_deg, Nmin, Nmax)
            
            if len(multiplets_max) > 0:
                for multiplet in multiplets_max:
                    if Nmin <= len(multiplet) <= Nmax:
                        
                        names = [name for name in events_table.colnames if len(events_table[name].shape) <= 1] # In case table has multidimensional columns, we must remove them
                        events_df = events_table[names].to_pandas()
                        find_photons = events_df.isin({"EVENT_ID": [photon.id_ for photon in multiplet]})
                        ...
                        da = make_circle(np.c_[multiplet_table["RA"], multiplet_table["DEC"]])[2] * 2
                        if (da < r_deg) & (dt < dt_threshold):
                            hits += 1
                            rows_multiplets.append(
                            [len(multiplet), multiplet_table["EVENT_ID"].data, multiplet_table["RA"].data,
                                multiplet_table["DEC"].data, multiplet_table["TIME"].data,
                                multiplet_table["ENERGY"].data, dt, da])
    ...
    if len(rows_multiplets) > 0:
        multiplets_table = Table(rows=rows_multiplets, names=('Nmax', 'ID', 'RA', 'DEC', 'TIME', 'ENERGY', 'dt', 'da'))

        return multiplets_table
    return []

def multiplet_scanner_simulated_table(target, events_lists, dt_threshold, r_deg, r_area, Nmin: int = 2, Nmax: int = None,
                      verbose=False):
    """Scans for Nmax-multiplets in specified sky region around target, blind search not implemented yet. 
    ...
    if target is not None:
        ...
        for counter, event_list in enumerate(events_lists):
            ...
            events_table['coord'] = SkyCoord(ra=events_table['RA'], dec=events_table['DEC'], unit='deg')
            events_table['fTIME'] = events_table['TIME'].copy().astype(np.float64)  # create a float64 time data column
            ...
            photons = [PhotonSim(t=event['fTIME'], ra=event['RA'], dec=event['DEC']) for event in
                    events_table] 
            ...
            if len(multiplets_max)>0:
                for multiplet in multiplets_max:
                    if Nmin <= len(multiplet) <= Nmax:
                        ...
                        multiplet_table = Table(rows=[[event.ra, event.dec, event.t] for event in multiplet],
                                                names=('RA', 'DEC', 'TIME'))
                        dt = (multiplet[-1].t - multiplet[0].t)
                        da = make_circle(np.c_[multiplet_table["RA"], multiplet_table["DEC"]])[2] * 2
                        if (da < r_deg) & (dt < dt_threshold):
                            hits += 1
                            rows_multiplets.append(
                            [len(multiplet), multiplet_table["RA"].data,
                                multiplet_table["DEC"].data, multiplet_table["TIME"].data, dt, da])
            if verbose:
                print(
                    f"\rScan no. {counter + 1}/{len(events_lists)} ({100 * (counter + 1) / len(events_lists):2.2f}%) \t| scanned {n_scanned} events so far \t| {Nmax}-multiplets: {hits} hits so far\r",
                    end='\r')
    ...
    if len(rows_multiplets) > 0:
        multiplets_table = Table(rows=rows_multiplets, names=('Nmax', 'RA', 'DEC', 'TIME', 'dt', 'da'))
        return multiplets_table
    else:
...
def worker(target, event_list, dt_threshold, r_deg, r_area, Nmin: int = 2, Nmax: int = None):
    ...
    if len(rows) > 0:    
        return Table(rows=rows, names=('Nmax', 'ID', 'RA', 'DEC', 'TIME', 'ENERGY', 'dt', 'da'))
    else:
        return None
...
def scanner_parallel(target, events_lists, dt_threshold, r_deg, r_area, Nmin: int = 2, Nmax: int = None,
                      verbose=False):
    ...
    with cf.ProcessPoolExecutor() as executor:
        future_multiplets = [executor.submit(worker, target, event_list, dt_threshold, r_deg, r_area, Nmin, Nmax) for
                             event_list in events_lists]
    if len(future_multiplets) > 0:
        return vstack([future.result() for future in future_multiplets if future.result() is not None])
    else:
        print('No multiplets found!')
        return None
...
def triplet_hist2d_plotter(table, bins=50, position_src=None, save=False):
    ...
    """
    figure, ((ax0, ax1, ax2)) = plt.subplots(1, 3, figsize=(18, 5),
    ...
    if position_src is not None:
        ...
        ra_1, dec_1 = table['RA'][:, 0] - position_src.ra.deg, table['DEC'][:, 0] - position_src.dec.deg

        ra_2, dec_2 = table['RA'][:, -1] - position_src.ra.deg, table['DEC'][:, -1] - position_src.dec.deg

        range_ra = np.max(
            np.abs(np.array(np.max(table['RA']) - position_src.ra.deg, np.min(table['RA']) - position_src.ra.deg)))
        range_dec = np.max(
            np.abs(np.array(np.max(table['DEC']) - position_src.dec.deg, np.min(table['DEC']) - position_src.dec.deg)))
    ...
    counts, xedges, yedges, im0 = ax0.hist2d(ra_avg, dec_avg, bins=bins, cmap=plt.cm.jet, range=range_)
    ...
    counts, xedges, yedges, im1 = ax1.hist2d(ra_2, dec_2, bins=bins, cmap=plt.cm.jet,
                                             # norm=norm
                                             range=range_)
    ...
    counts, xedges, yedges, im2 = ax2.hist2d(ra_1, dec_1, bins=bins, cmap=plt.cm.jet,
                                             # norm=norm
                                             range=range_)
